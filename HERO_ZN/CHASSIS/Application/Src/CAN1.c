...
void RF_Motor_Disconnect(void){Motor_online[RF] = 0;}
void RB_Motor_Disconnect(void){Motor_online[RB] = 0;}
void LB_Motor_Disconnect(void){Motor_online[LB] = 0;}
void LF_Motor_Disconnect(void){Motor_online[LF] = 0;}

void RF_Motor_Online(void)
{
    Motor_online[RF] = 1;
    Get_Motor_Measure_Data(&RF_Motor,CAN1_Rx_Data);
    CAN_DATA_Encoder_Deal(RF_Motor.position_data, RF_Motor.speed_data, &CAN_Encoder[CHASSIS_MOTOR_RF_ENCODER]);
}
void RB_Motor_Online(void){Motor_online[RB] = 1;}
void LB_Motor_Online(void){Motor_online[LB] = 1;}
void LF_Motor_Online(void){Motor_online[LF] = 1;}
...
//    CAN_Motor.last_call_back_time = last_call_back_time;
    ...
    Safe_task_add("RF_Motor",40, RF_Motor_Disconnect, RF_Motor_Online);
//    Safe_task_add("RB_Motor",40, RB_Motor_Disconnect, RB_Motor_Online);
//    Safe_task_add("LB_Motor",40, LB_Motor_Disconnect, LB_Motor_Online);
//    Safe_task_add("LF_Motor",40, LF_Motor_Disconnect, LF_Motor_Online);
}
