...
Encoder_t * get_pitch_motor_encoder_point (void)
{
    return &pitch_encoder;
}
Encoder_t * get_yaw_motor_encoder_point (void)
{
    return &yaw_encoder;
}
...
*/
void Encoder_Init(Encoder_t * Temp_Encoder, Encoder_Type_e Encoder_Type)
{
...
void CAN_DATA_Encoder_Deal(int16_t position, int16_t speed, Encoder_t * Encoder) {
   ...
   Encoder->Radian=((float)((Encoder->Encode_Actual_Val * Encoder->gear_Ratio) / Encoder->lap_encoder))*2*PI;
   Encoder->Total_Radian = ((float)((Encoder->Encode_Record_Val * Encoder->gear_Ratio) / Encoder->lap_encoder))*2*PI;
