...
void Motor_information_init(void)
{
    ...
    /*初始化电机编码器数据*/
    New_Chassis_Data.Motor_information.Motor_encoder[CHASSIS_MOTOR_RF_ENCODER]   = Get_Chassis_motor_encoder_point(CHASSIS_MOTOR_RF_ENCODER);
    New_Chassis_Data.Motor_information.Motor_encoder[CHASSIS_MOTOR_RB_ENCODER]   = Get_Chassis_motor_encoder_point(CHASSIS_MOTOR_RB_ENCODER);
    New_Chassis_Data.Motor_information.Motor_encoder[CHASSIS_MOTOR_LB_ENCODER]   = Get_Chassis_motor_encoder_point(CHASSIS_MOTOR_LB_ENCODER);
    New_Chassis_Data.Motor_information.Motor_encoder[CHASSIS_MOTOR_LF_ENCODER]   = Get_Chassis_motor_encoder_point(CHASSIS_MOTOR_LF_ENCODER);
    New_Chassis_Data.Motor_information.Motor_encoder[CHASSIS_MOTOR_FIRE_ENCODER] = Get_Chassis_motor_encoder_point(CHASSIS_MOTOR_FIRE_ENCODER);
...
/*
 *函数名称：Motor_Online_Detection
 *函数参数：void
 *函数返回：void
 *函数功能：根据电机失联时间长短判断是否掉线
 *特别说明：只有底盘四电机，无拨盘电机
 */
void Motor_Online_Detection(void)
{
    
    for(int i = 0; i < 5;i ++)    (New_Chassis_Data.Motor_information.last_call_back_time[i])++;
    //未添加拨盘电机掉线检测(￣_,￣ )
    if (New_Chassis_Data.Motor_information.last_call_back_time[RF] > 5000)    New_Chassis_Data.Motor_information.motor_state = RF_MISS;
    if (New_Chassis_Data.Motor_information.last_call_back_time[LF] > 5000)    New_Chassis_Data.Motor_information.motor_state = LF_MISS;
    if (New_Chassis_Data.Motor_information.last_call_back_time[LB] > 5000)    New_Chassis_Data.Motor_information.motor_state = LB_MISS;
    if (New_Chassis_Data.Motor_information.last_call_back_time[RB] > 5000)    New_Chassis_Data.Motor_information.motor_state = RB_MISS;
    
    switch(New_Chassis_Data.Motor_information.motor_state)//后续掉线电机重新上线检测
    {
        case ALL_OK:
            break;
        case RF_MISS:
            if (Chassis_Data.CAN1_Chassis->last_call_back_time[RF] < 5000)    New_Chassis_Data.Motor_information.motor_state = ALL_OK;
            break;
        case LF_MISS:
            if (Chassis_Data.CAN1_Chassis->last_call_back_time[LF] < 5000)    New_Chassis_Data.Motor_information.motor_state = ALL_OK;
            break;
        case LB_MISS:
            if (Chassis_Data.CAN1_Chassis->last_call_back_time[LB] < 5000)    New_Chassis_Data.Motor_information.motor_state = ALL_OK;
            break;
        case RB_MISS:
            if (Chassis_Data.CAN1_Chassis->last_call_back_time[RB] < 5000)    New_Chassis_Data.Motor_information.motor_state = ALL_OK;
            break;
        default:
			break;
    }
